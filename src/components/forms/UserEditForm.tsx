...
export default function UserEditForm({ id, onSuccess, onCancel }: UserEditFormProps) {
  ...
  useEffect(() => {
    async function loadUser() {
      try {
        ...
        setRoleKey(userData.role_key || 'employee');
  ...
  if (initLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-6 space-y-2">
        ...
        <span className="text-sm text-slate-500">Retrieving user profile...</span>
      </div>
    );
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex items-center pt-2">
        <label className="inline-flex items-center space-x-2 cursor-pointer">
          ...
          />
          <span className="text-sm font-semibold text-slate-700">Account is active and allowed login access</span>
        </label>
      </div>
