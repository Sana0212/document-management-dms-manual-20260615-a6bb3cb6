...
interface RetentionPolicyEditFormProps {
  policyId: string;
  onSuccess?: (id: string) => void;
  onCancel?: () => void;
}

export default function RetentionPolicyEditForm({ policyId, onSuccess, onCancel }: RetentionPolicyEditFormProps) {
  ...
  useEffect(() => {
    async function loadPolicyDetails() {
      try {
        const res = await fetch(`/api/retention_policies/${policyId}`);
        if (res.ok) {
          const p = (await res.json()) as RetentionPolicy;
          setName(p.name || '');
          setCode(p.code || '');
          setDescription(p.description || '');
          setDurationValue(p.duration_value || 1);
          setDurationUnit(p.duration_unit || 'years');
          setLegalHoldAllowed(p.legal_hold_allowed !== false);
          setIsDefault(p.is_default !== false);
          setIsActive(p.is_active !== false);
          setCreatedAt(p.created_at || new Date().toISOString());
        } else {
          setError('Failed to fetch the specified retention policy.');
        }
      } catch (err) {
        console.error('Failed to load retention policy details', err);
        setError('Error loading policy values.');
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errJson.error || 'Failed to update retention policy');
      }
  ...
  if (initLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-6 space-y-2">
        ...
        <span className="text-sm text-slate-500">Loading policy parameters...</span>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <textarea
          rows={2}
          placeholder="Detailed description of the retention terms..."
          className="w-full px-3 py-2 text-sm bg-white border border-slate-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500"
          value={description}
